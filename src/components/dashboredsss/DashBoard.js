...
import { AGGREGATE, LEAVEAPPLY, REPORTS } from "../../serverconfiguration/controllers";
import { getRequest, postRequest } from "../../serverconfiguration/requestcomp";

export default function DashBoard() {
  const [data, setData] = useState([]);
  const [graphData, setGraphData] = useState([]);
  const [notificationsData, setNotificationsData] = useState([]); // Corrected initialization
  ...
  useEffect(() => {
    ...
    async function getNotificationsData() {
      return postRequest(ServerConfig.url, REPORTS, {
        query: "SELECT COUNT(*) FROM leave_apply WHERE status = 'P';",
      });
    }
    getData().then((e) => {
      setData(e.data);
      getGraphData().then((s) => {
        setGraphData(s.data);
        getNotificationsData().then((n) => {
          setNotificationsData(n.data); // Corrected function call
        });
      });
    });
  }, []);
  ...
  useEffect(() => {
    ...
    // Set up an interval to refresh the page every 10 seconds
